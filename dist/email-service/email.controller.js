...
const simple_notification_dto_1 = require("./dto/simple-notification.dto");
let EmailController = class EmailController {
    ...
    async sendNotification(notificationDto) {
        const { to, subject, message, name, actionUrl, actionText } = notificationDto;
        const result = await this.emailService.sendNotification(to, subject, message, name, actionUrl, actionText);
        return new api_response_1.ApiResponse(result);
    }
    async sendSimpleNotification(notificationDto) {
        const result = await this.emailService.sendSimpleNotification(notificationDto);
        return new api_response_1.ApiResponse(result);
    }
};
...
__decorate([
    (0, common_1.Post)('send-simple'),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [simple_notification_dto_1.SimpleNotificationDto]),
    __metadata("design:returntype", Promise)
], EmailController.prototype, "sendSimpleNotification", null);
